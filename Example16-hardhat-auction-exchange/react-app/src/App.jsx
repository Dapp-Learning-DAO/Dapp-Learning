...
function App(props) {
  ...
  for(let a in assetsAuctionStat){
    yourCollectibles[assetsAuctionStat[a]] = true
  }
  ...
  */
  ...
  const updateYourCollectibles = async () => {
    ...
    for(let a in assets){
      try {
        const forSale = yourCollectibles[a]
        let owner
        let auctionInfo
        if(!forSale){
  ...
  const mintItem = async (tokenUri) => {
    let total = await writeContracts.MYERC721.totalSupply()
    let hash = await readContracts.MYERC721.mintWithTokenURI(address, tokenUri)
    try {
      let total = await readContracts.MYERC721.totalSupply()
    } catch (e) {
      console.log("error========")
      console.log(e)
    }
      ...
      //yourCollectibles[tokenUri] = false
      //setYourCollectibles(yourCollectibles)
    console.log("sss")
  }
