...
- 当价格来到一个 tick 区间内，我们只要记录这个区间内的交易量，根据费率算出该区间的总手续费
- 然后找出所有包含该 tick 的流动性头寸 `position`，先将他们的数量汇总
...
上述方案一个较大的问题在于让交易者承担了做市者的 gas 费用，因为记录手续费的分配细节，是做市商们的事务。对于交易者来说，本就承担了手续费，然后还要负责耗费大量 gas 去帮他的对手盘们记录怎么分钱，这肯定不是什么美妙的体验。其实就算是做市商也不会愿意承受。

我们需要把分配的计算过程放到注入和移除流动性的阶段，而不是在每次交易的过程中：

- 在每个 `position` 注入的时候记录当时区间内的手续费数量
- 然后在移除的时候，计算此时的手续费比之前增加了多少，即该 `position` 参与做市期间，区间内增长的手续费总量
- 最后再根据出资比例去分配收益，就能保证公平性

所以，我们剩下一个最棘手的问题待解决——如何在不去遍历区间内所有 `tick` 的情况下，计算出该区间的手续费？

### 区间外侧的手续费

如果我们只关注价格区间内的变量，几乎是无解的，因为在每一根 tick 上增加的手续费都是不同的情况（每一根 tick 上的流动性数量不同），除了遍历别无他法。如果是窄的价格区间还好，若是一个 (0, ∞) 的区间，岂不是要遍历所有的 tick？

当我们把目光移到价格区间之外，会发现有一个变量在交易的过程中是不变的。
...
```math
fee_inside = feeGrowthGlobal - fee_outside = feeGrowthGlobal - fee_outside_a - fee_outside_b
```

池子的总手续费，只要在每次交易的过程中累加就好了，我们只需要更新一个全局变量 `feeGrowthGlobal`。而 `fee_outside_a` 和 `fee_outside_b` 的值则需要在 tick 上增加一个 `feeGrowthOutside` 变量。

每当有流动性注入或移除的时候，在价格的边界对应的 tick 上有如下更新规则：
...
而在交易过程中，`feeGrowthOutside` 有如下更新规则：

- 当价格穿过某个 tick 时，该 tick 上的 `feeGrowthOutside` 需要翻转，因为外侧手续费永远要在当前价格的另一侧，固 `feeGrowthOutside = feeGrowthGlobal - feeGrowthOutside`

遵照如上规则，我们可以模拟一下手续费的计算过程。首先在注入流动性的时候，a 和 b 点的tick上，外侧手续费就已经确定。

- 当一笔交易在区间内进行，全局总手续费会增加，但外侧的手续费不会增加，通过全局减两边的外侧可以获得区间内手续费
- 当价格移动到区间外，比如b点右侧，`feeGrowthOutside` 需要翻转，

那么价格区间左侧的外部手续费 `fee_outside_a = feeGrowthOutside_`
