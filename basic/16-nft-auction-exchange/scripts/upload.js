...
const main = async () => {
  ...
  for(let a in artwork){
    ...
    var buf = Buffer.from(stringJSON,'utf-8');
    const uploaded = await ipfs.add(buf)
    console.log("   "+artwork[a].name+" ipfs:",uploaded[0].path)
    allAssets[uploaded[0].path] = artwork[a]
  }
