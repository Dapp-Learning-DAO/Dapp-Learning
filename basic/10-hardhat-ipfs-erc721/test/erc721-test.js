...
const fromWei = (value) => formatEther(typeof value === 'string' ? value : value.toString());
...
describe('MYERC721 contract', function () {
  ...
  it('deployer has the default admin role', async function () {
    ...
    expect(await token.getRoleMember(DEFAULT_ADMIN_ROLE, 0)).to.equal(deployer.address);
  });

  it('deployer has the minter role', async function () {
    expect(await token.getRoleMemberCount(MINTER_ROLE)).to.be.equal('1');
    expect(await token.getRoleMember(MINTER_ROLE, 0)).to.equal(deployer.address);
  });
  ...
  describe('minting', function () {
    ...
    it('other accounts cannot mint tokens', async function () {
      await expect(token.connect(other).mint(other.address)).to.be.revertedWith('ERC721PresetMinterPauserAutoId: must have minter role to mint');
    });
  });

  describe('pausing', function () {
    it('deployer can pause', async function () {
      await expect(token.pause()).to.emit(token, 'Paused').withArgs(deployer.address);
    ...
    it('deployer can unpause', async function () {
      await token.pause();
      await expect(token.unpause()).to.emit(token, 'Unpaused').withArgs(deployer.address);
    ...
    it('cannot mint while paused', async function () {
      ...
      await expect(token.connect(deployer).mint(other.address)).to.be.revertedWith('ERC721Pausable: token transfer while paused');
    });

    it('other accounts cannot pause', async function () {
      await expect(token.connect(other).pause()).to.be.revertedWith('ERC721PresetMinterPauserAutoId: must have pauser role to pause');
    });

    it('other accounts cannot unpause', async function () {
      await token.pause();

      await expect(token.connect(other).unpause()).to.be.revertedWith('ERC721PresetMinterPauserAutoId: must have pauser role to unpause');
    });
  });
  ...
  describe('burning', function () {
    it('holders can burn their tokens', async function () {
      ...
      await expect(token.connect(other).burn(tokenId)).to.emit(token, 'Transfer').withArgs(other.address, ZERO_ADDRESS, tokenId);
